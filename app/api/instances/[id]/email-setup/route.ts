import { NextResponse } from 'next/server'
import { resolveInstance } from '@/lib/instances'
import { runCommand } from '@/lib/ssh'
import { generateEmailSetupScript } from '@/lib/email-setup'

export async function POST(
  req: Request,
  { params }: { params: Promise<{ id: string }> }
) {
  const { id } = await params
  try {
    const { email, agentId } = await req.json()
    if (!email || !agentId) {
      return NextResponse.json({ error: 'email and agentId are required' }, { status: 400 })
    }

    const instance = await resolveInstance(id)
    if (!instance) return NextResponse.json({ error: 'Instance not found' }, { status: 404 })

    const config = { host: instance.ip, privateKey: instance.sshKey }
    const script = generateEmailSetupScript(email, agentId)

    const writeResult = await runCommand(
      config,
      `cat > /tmp/reef-email-setup.sh << 'REEF_SCRIPT_EOF'\n${script}\nREEF_SCRIPT_EOF`
    )
    if (writeResult.code !== 0) {
      return NextResponse.json({ success: false, error: `Failed to upload script: ${writeResult.stderr}` }, { status: 500 })
    }

    await runCommand(config, 'chmod +x /tmp/reef-email-setup.sh')

    return NextResponse.json({ success: true })
  } catch (err) {
    return NextResponse.json(
      { success: false, error: err instanceof Error ? err.message : 'Unknown error' },
      { status: 500 }
    )
  }
}
